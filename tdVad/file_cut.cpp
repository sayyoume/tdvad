...
int FileCut::cut_write_file(struct cut_info *cut, int frames) {
    ...
    if (readed > 0) {
        FILE* res_file = NULL;
        fopen_s(&res_file,cut->result_filename, "wb+");
        if (res_file == NULL) {
...
int FileCut::cut_frame(struct cut_info *cut, int last_frame, int force) {
...
int FileCut::add_continued(struct cut_info *cut, int is_active) {
...
int FileCut::add_changed(struct cut_info *cut, int is_active) {
    int frame = 0;
    if (is_active) {
        // 连续静音，之后遇到响声
...
struct cut_info * FileCut::cut_info_create(FILE *fp) {
    struct cut_info *cut = (cut_info*)calloc(1, sizeof(struct cut_info));
    cut->fp = fp;
    return cut;
}


int FileCut::cut_add_vad_activity(struct cut_info *cut, int is_active, int is_last) {
    int res;
    if (is_last ||
...
void FileCut::cut_info_free(struct cut_info *cut) {
    free(cut);
}


void FileCut::cut_info_print(struct cut_info *cut) {
