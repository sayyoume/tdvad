...
int SimpleVad::read_int16_bytes(FILE *fp, int16_t *output) {
    if (feof(fp)) {
        return 1;
    }
...
VadInst* SimpleVad::simple_vad_create() {
...
int SimpleVad::process_vad(VadInst *inst, int16_t *data) {
    int res = WebRtcVad_Process(inst, SAMPLE_RATE, data, FRAME_SIZE);
    return res;
}

void SimpleVad::simple_vad_free(VadInst* inst) {
    WebRtcVad_Free(inst);
}
