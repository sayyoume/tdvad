...
int PeroidFormat::add_period_start(struct periods *per) {
    per->size++;
    if (per->size > per->size_allocated) {
        size_t new_size = per->size_allocated * 2 * sizeof(uint64_t);
        per->period_start = (uint64_t*)realloc(per->period_start, new_size);
        if (per->period_start == NULL) {
            fprintf(stderr, "per->period_start allocated failed %ld", new_size);
            return 1;
        }
        per->period_end = (uint64_t*)realloc(per->period_end, new_size);
...
void PeroidFormat::add_period_end(struct periods *per) {
    size_t end_time = CAL_TIME_BY_FRAME(per->current_frame);
    per->period_end[per->size - 1] = end_time;
    per->is_end_filled = 1;
}

struct periods * PeroidFormat::periods_create() {
    struct periods *per = (periods*)calloc(1, sizeof(struct periods));
    if (per == NULL) {
        fprintf(stderr, "per alloc failed");
        return NULL;
    }
    per->period_start = (uint64_t*)calloc(PERIODS_SIZE_INITIED, sizeof(uint64_t));
    if (per->period_start == NULL) {
        fprintf(stderr, "per->period_start alloc failed");
    } else {
        per->period_end = (uint64_t*)calloc(PERIODS_SIZE_INITIED, sizeof(uint64_t));
        if (per->period_end == NULL) {
            fprintf(stderr, "per->period_start alloc failed");
        } else {
...
void PeroidFormat::periods_print(struct periods *per) {
    int size = per->size;
...
int PeroidFormat::period_add_vad_activity(struct periods *per, int is_active, int is_last) {
...
void PeroidFormat::periods_free(struct periods *per) {
    free(per->period_start);
    free(per->period_end);
    free(per);
}

PeroidFormat::PeroidFormat()
{
