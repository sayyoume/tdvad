...
public:
	...
	BOOL SetOleCallback(IRichEditOleCallback* pCallback);    void SetClientRect(RECT *prc);
...
CTxtWinHost::CTxtWinHost() : m_re(NULL), pserv(nullptr)
{
...
ITextHost * CTxtWinHost::GetTextHost()
{
	AddRef();
	return this;
}

ITextServices * CTxtWinHost::GetTextServices2()
{
	if (NULL == pserv)
		return NULL;
	pserv->AddRef();
...
BOOL CTxtWinHost::SetOleCallback(IRichEditOleCallback* pCallback)
{
	if (NULL == pserv)
		return FALSE;
	HRESULT lRes = 0;
...
void CTxtWinHost::PasteSpecial(UINT uClipFormat, DWORD dwAspect /*= 0*/, HMETAFILE hMF /*= 0*/)
{
	if (NULL == pserv)
		return;
...
{ 
	...
	//无格式粘贴
	//PasteSpecial(CF_UNICODETEXT);
    
}

void CRichEditUI::Delete()
{
	TxSendMessage(WM_CLEAR, 0, 0, 0);
}
...
HRESULT CRichEditUI::TxSendMessage(UINT msg, WPARAM wparam, LPARAM lparam, LRESULT *plresult) const
{
    if( m_pTwh ) {
		...
		ITextServices* iText = m_pTwh->GetTextServices();
		if (iText != nullptr)
		{
			return iText->TxSendMessage(msg, wparam, lparam, plresult);
		}
		else
		{
			int ii = 0;
...
void CRichEditUI::FindRichEditText(std::wstring& sText, const int& inStart, const int& inEnd, int& outStart, int& outEnd)
{
	LRESULT lResult;

	FINDTEXTEX pf;
	pf.lpstrText = sText.c_str();
	
	pf.chrg.cpMin = inStart;// +wcslen(pf.lpstrText);
	pf.chrg.cpMax = -1;
				  
	TxSendMessage(EM_FINDTEXTEXW, FR_DOWN  | FR_MATCHCASE, (LPARAM)&pf, &lResult);

	outStart = pf.chrgText.cpMin;
	outEnd = pf.chrgText.cpMax;
}
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
    ...
    if( (uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR ) {
        ...
        if( uMsg == WM_SETCURSOR ) bWasHandled = false;
            ...
            SetFocus();
			Invalidate(); //add 2017-6-5 qinxiaowei  解决edit失去焦点再获取焦点 光标没没消失 
        }
    }
#ifdef _UNICODE
...
void CRichEditUI::PasteSpecial(UINT uClipFormat, DWORD dwAspect, HMETAFILE hMF)
{
	if (NULL == m_pTwh)
		return;
...
void CRichEditUI::SetPassWord(bool bPassword)
{
	if (bPassword)
	{
		m_lTwhStyle |= ES_PASSWORD;
