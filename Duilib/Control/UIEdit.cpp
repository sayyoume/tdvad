...
namespace DuiLib
{
	...
	CEditUI::CEditUI() : m_pWindow(NULL), m_uMaxChar(255), m_bReadOnly(false), 
		...
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0), m_dwTipTextColor(4287206044)
	{
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("tiptextcolor")) == 0)
		{
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwTipTextColor = _tcstoul(pstrValue, &pstr, 16);
			int ii = 3;
		}
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if (m_sText.IsEmpty() && m_TipsText.IsEmpty()) return;

		CDuiString sText = m_sText;
		std::wstring strText = GetText();
		if (m_pManager->GetFocus() != this && strText.empty())
		{
			sText = m_TipsText;
			m_dwTextColor = m_dwTipTextColor;
		}
		else
		{
			if (m_dwTextColor == 0) m_dwTextColor = m_pManager->GetDefaultFontColor();
			m_dwTextColor = m_oldTextColor;
			sText = m_sText;
	...
	void CEditUI::SetTipText(LPCTSTR pstrValue)
	{
		m_TipsText = pstrValue;
	}
