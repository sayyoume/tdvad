...
void CPaintManagerUI::ReloadLanguage(UINT uNotifyMsg /*= 0*/)
{
	for (int i = 0; i < m_aPreMessages.GetSize(); i++) {
		CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
		pManager->ReloadLanguageID(dynamic_cast<CContainerUI*>(pManager->GetRoot()));
		pManager->GetRoot()->NeedUpdate();
		if (uNotifyMsg != 0) PostMessage(pManager->GetPaintWindow(), uNotifyMsg, 0, 0);
	}
}

void CPaintManagerUI::ReloadLanguageID(CContainerUI *pContainer /*= NULL*/)
{
	...
	for (int i = 0; i < pContainer->GetCount(); ++i)
	{
		...
		// textid
		short sTextID = pControl->GetTextID();
		if (sTextID != 0) pControl->SetTextID(sTextID);
		// tipid
		short sTooltipID = pControl->GetToolTipId();
		if (sTooltipID != 0) pControl->SetToolTipId(sTooltipID);

		CContainerUI *pSubContainer = dynamic_cast<CContainerUI*>(pControl);
		if (pSubContainer != NULL) ReloadLanguageID(pSubContainer);
	}
}
...
SIZE CPaintManagerUI::GetRoundCorner() 
{
	...
	return GetDPIObj()->Scale(szRoundCorner);
}
...
SIZE CPaintManagerUI::GetMinInfo() 
{
	...
	return GetDPIObj()->Scale(szMinWindow);
...
SIZE CPaintManagerUI::GetMaxInfo()
{
    ...
    return GetDPIObj()->Scale(szMaxWindow);
}
...
void CPaintManagerUI::RebuildFont(TFontInfo* pFontInfo)
{
...
void CPaintManagerUI::SetDPI(int iDPI)
{
