...
namespace DuiLib
{
	...
	int CContainerUI::GetBkImageW()
	{
		if (m_diBk.pImageInfo) {
			return m_diBk.pImageInfo->nX;
		}
		return 0;
	}

	int CContainerUI::GetBkImageH()
	{
		if (m_diBk.pImageInfo) {
			return m_diBk.pImageInfo->nY;
		}
		return 0;
	}
	...
	void CContainerUI::Sort(bool(pFun(CControlUI* pControl1, CControlUI* pControl2)))
	{
		bool bSort = false;
		for (int i = 0; i < m_items.GetSize() - 1; ++i)
		{
			for (int j = i + 1; j < m_items.GetSize(); ++j)
			{
				CControlUI* pSubControl1 = static_cast<CControlUI*>(m_items.GetAt(i));
				CControlUI* pSubControl2 = static_cast<CControlUI*>(m_items.GetAt(j));
				if (pFun(pSubControl1, pSubControl2))
				{
					bSort = true;
					m_items.SetAt(i, pSubControl2);
					m_items.SetAt(j, pSubControl1);
				}
			}
		}
