...
void CControlUI::SetTextID(short sId)
{
	std::wstring strText = UG::Lang::getLang(sId);
	SetText(strText.c_str());
	m_sTextID = sId;
}

short CControlUI::GetTextID()
{
	return m_sTextID;
}
...
SIZE CControlUI::GetBorderRound() const
{
   // return m_cxyBorderRound;
	SIZE rcBorderSize = m_cxyBorderRound;
	if (m_pManager != NULL) m_pManager->GetDPIObj()->Scale(&rcBorderSize);
	return rcBorderSize;
}
...
void CControlUI::SetToolTipId(short sId)
{
	std::wstring strText = UG::Lang::getLang(sId);
	SetToolTip(strText.c_str());
	m_sTooltipID = sId;
}

short CControlUI::GetToolTipId()
{
	return m_sTooltipID;
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcsicmp(pstrName, _T("textid")) == 0) SetTextID(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, _T("tooltip")) == 0 ) SetToolTip(pstrValue);
	else if (_tcsicmp(pstrName, _T("tooltipid")) == 0) SetToolTipId(_ttoi(pstrValue));
...
SIZE CControlUI::EstimateSize(SIZE szAvailable)
{
	SIZE cxyFixed = m_cxyFixed;
	if (m_pManager != NULL)
		 m_pManager->GetDPIObj()->Scale(&cxyFixed);
	return cxyFixed;
}
